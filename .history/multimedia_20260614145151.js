...
videoEl.addEventListener('timeupdate', () => {
    ...
    // Buscar todos los hotspots que aún no son visibles
    const hiddenHotspots = hotspotLayer.querySelectorAll('.docu-hotspot:not(.visible)');
    
    hiddenHotspots.forEach(dot => {
        const triggerTime = parseFloat(dot.dataset.time);
        
        // Si el video llegó al segundo indicado, mostrar el hotspot
...
function renderHotspots(hotspots) {
    ...
    hotspots.forEach((spot, index) => {
        ...
        // Tiempo de desaparición: el startTime del siguiente hotspot
        // Si es el último, desaparece 5 segundos antes del final del video
        const nextSpot = hotspots[index + 1];
        if (nextSpot) {
            dot.dataset.endTime = nextSpot.time;
        } else {
            // Para el último hotspot, usamos el final del video menos 5s

...
// === CARRUSEL DE ARTISTAS CON LOOP INFINITO (BLINDADO) ===
(function() {
    'use strict';
    
    // 🛡️ GUARD GLOBAL: Evita ejecución duplicada del script
    if (window.__artistsCarouselInitialized) {
        console.log('⚠️ Carrusel artistas ya inicializado, saltando...');
        return;
    }
    window.__artistsCarouselInitialized = true;
    ...
    artistTracks.forEach(track => {
        ...
        // 🛡️ PROTECCIÓN 1: Verificar si ya fue clonado (por dataset)
        if (track.dataset.cloned === 'true') {
            console.log('⚠️ Track ya clonado por dataset, saltando...');
            return;
        }
        
        // 🛡️ PROTECCIÓN 2: Verificar si ya hay clones (por cantidad de hijos)
        const currentChildren = track.querySelectorAll('.artists-carousel-items').length;
        if (currentChildren > 1) {
            console.log('⚠️ Track ya tiene clones, saltando...');
        ...
        const updateAnimation = () => {
            ...
            const oldStyles = document.querySelectorAll(`style[data-animation="${animationName}"]`);
    ...
    console.log('✅ Carrusel de artistas inicializado correctamente');
})();
// === MODAL DE ARTISTA ===

...
// === CARRUSEL DE ARTISTAS - VERSIÓN ROBUSTA ===
(function() {
    'use strict';
    
    // Variable global para evitar ejecución múltiple
    if (window.artistCarouselInitialized) {
        console.log('⚠️ Carrusel de artistas ya inicializado, saltando...');
        return;
    }
    ...
    function initArtistCarousel() {
        ...
        if (!track) {
            console.error('❌ No se encontró .artists-carousel-track');
            return;
        }
        
        const items = track.querySelector('.artists-carousel-items');
        if (!items) {
            console.error('❌ No se encontró .artists-carousel-items');
            return;
        }
        
        // Eliminar clones existentes si los hay
        const existingClones = track.querySelectorAll('.artists-carousel-items[aria-hidden="true"]');
        ...
        console.log('✅ Carrusel de artistas inicializado con', items.children.length, 'tarjetas');
        
        // Función para calcular y aplicar animación
        function applyAnimation() {
            const itemsWidth = items.scrollWidth;
            const gap = 30;
            const totalWidth = itemsWidth + gap;
            
            // Crear keyframes únicos
            const animName = 'artistScroll_' + Date.now();
            ...
            // Aplicar animación
            items.style.animation = `${animName} 40s linear infinite`;
            clone.style.animation = `${animName} 40s linear infinite`;
            
            console.log('🎬 Animación aplicada:', totalWidth, 'px');
        }
        ...
        window.artistCarouselInitialized = true;
    }
    
    // Ejecutar cuando el DOM esté listo
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initArtistCarousel);
    } else {
        initArtistCarousel();
    }
})();
...
(function() {
    ...
    if (window.artistModalInitialized) return;
    ...
    function initModal() {
        ...
        console.log('✅ Modal de artista inicializado');
        ...
        window.artistModalInitialized = true;
    }

...
function showFlashTitle(spotData) {
    ...
    const topPercent = parseFloat(spotData.top);
    const leftPercent = parseFloat(spotData.left);
    
    let finalTop, finalLeft;
    
    // Si está muy arriba (menos del 25%), mostrar DEBAJO del punto
    if (topPercent < 25) {
        finalTop = `calc(${spotData.top} + 40px)`;
    } 
    // Si está muy abajo (más del 75%), mostrar más arriba
    else if (topPercent > 75) {
        finalTop = `calc(${spotData.top} - 100px)`;
    } 
    // Normal: mostrar encima
    else {
        finalTop = `calc(${spotData.top} - 60px)`;
    }
    
    // Si está muy a la izquierda (menos del 20%), mover hacia el centro
    if (leftPercent < 20) {
        finalLeft = '25%';
    } 
    // Si está muy a la derecha (más del 80%), mover hacia la izquierda
    else if (leftPercent > 80) {
        finalLeft = '75%';
    } 
    // Centro: mantener posición original
    else {
        finalLeft = spotData.left;
    }
    
    flashEl.style.top = finalTop;
    flashEl.style.left = finalLeft;
